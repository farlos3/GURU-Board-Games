...
import howToPlayData from "/src/pages/howToPlay.json"; // ข้อมูลวิธีการเล่น
...
const HowToPlay = () => {
  ...
  useEffect(() => {
    if (id && games[id]) {
      ...
      // หาข้อมูลวิธีการเล่นของเกมนี้
      // สมมติว่า howToPlay.json มี structure เป็น array หรือ object ที่ key เป็น id หรือ name
      // ถ้า howToPlayData เป็น array
      const howToPlayInfo = howToPlayData.find(item => 
        item.id === parseInt(id) || item.name === game.name
      );
      
      // หรือถ้า howToPlayData เป็น object ที่ใช้ id เป็น key
      // const howToPlayInfo = howToPlayData[id];
      
      setGameHowToPlay(howToPlayInfo);
    }
  }, [id]);
    ...
    <>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {/* แสดงวิธีการเล่น */}
          {gameHowToPlay.howToplay && gameHowToPlay.howToplay.map((section, index) => (
            <div key={index}>
          ...
          {(!gameHowToPlay.howToplay || gameHowToPlay.howToplay.length === 0) && (
