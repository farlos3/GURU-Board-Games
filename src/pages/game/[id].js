...
import games from "/src/pages/Real-data.json"; // Import JSON data
...
function GameDetail() {
  ...
  const saveGameStatesToStorage = (states) => {
    try {
      ...
      // เพิ่มบรรทัดนี้เพื่อส่ง event ไปยังหน้าอื่น
  ...
  // แทนที่ useEffect ตัวแรกในไฟล์ paste.txt (บรรทัดที่ 42-75) ด้วยโค้ดนี้
  useEffect(() => {
    if (id && games[id]) {
      ...
      const similar = games
        .filter(
          (g, index) =>
            index != id && g.tags.some((tag) => game.tags.includes(tag))
        )
      ...
      // ใช้ข้อมูลที่มีอยู่แล้วใน localStorage แทนการสร้างใหม่
      // เพิ่มเฉพาะเกมที่ยังไม่มีใน savedStates
      const updatedStates = { ...savedStates };

      // เกมปัจจุบัน - เพิ่มเฉพาะถ้ายังไม่มี
      if (!updatedStates[id]) {
        updatedStates[id] = {
          isFavorite: false,
          isLiked: false,
      ...
      // เกมที่คล้ายกัน - เพิ่มเฉพาะถ้ายังไม่มี
      similar.forEach((game) => {
        const gameIndex = games.findIndex((g) => g.name === game.name);
        if (!updatedStates[gameIndex]) {
          updatedStates[gameIndex] = {
            isFavorite: false,
            isLiked: false,
            userRating: 0,
          };
                    ...
                    <div className={styles.item_game_text}>
                      ...
                      <div className={styles.rating_buttons}>
                        <button
                          ...
                          title={
                            gameStates[gameIndex]?.isLiked ? "Unlike" : "Like"
                          }
                        >
                          {gameStates[gameIndex]?.isLiked ? "💖" : "🤍"}
                        </button>
                        ...
                        <button
                          className={`${styles.favorite_button} ${
                            gameStates[gameIndex]?.isFavorite
